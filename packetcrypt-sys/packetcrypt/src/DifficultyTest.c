...
__global__ void setuint64Kernal (unsigned char* d_p, bool fLeadingZeroes, uint64_t n) {
...
static void setuint64(BIGNUM* out, uint64_t n)
{
    ...
    unsigned char* d_p;
    cudaMalloc((void **)&d_p, sizeof(char));
    cudaMemcpy(d_p, p, sizeof(char), cudaMemcpyHostToDevice);
    setuint64Kernal<<<1, 8>>>(d_p, fLeadingZeroes, n);
    cudaMemcpy(p, d_p, sizeof(char), cudaMemcpyDeviceToHost);
