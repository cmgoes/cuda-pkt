...
__global__ int checkPcHashKernal (CryptoCycle_State_t* d_pcState, uint64_t* d_indexesOut, PacketCrypt_HeaderAndProof_t* d_hap) {
    int index = threadIdx.x + blockIdx.x * blockDim.x;    
    // This gets modded over the total anns in PacketCryptProof_hashProof()
    indexesOut[index] = CryptoCycle_getItemNo(&d_pcState);
    CryptoCycle_Item_t* it = (CryptoCycle_Item_t*) &d_hap->announcements[index];
    if (Util_unlikely(!CryptoCycle_update(&d_pcState, it))) { return -1; }    
}

__global__ int Validate_checkBlockKernal (PacketCrypt_HeaderAndProof_t* d_hap, uint8_t* d_blockHashes, Buf32_t* d_annHashes, 
PacketCrypt_ValidateCtx_t* d_vctx, PacketCrypt_Coinbase_t* d_coinbaseCommitment, uint32_t blockHeight) {
    int index = threadIdx.x + blockIdx.x * blockDim.x;
    const PacketCrypt_Announce_t* ann = &d_hap->announcements[index];
    if (blockHashes != NULL) {
        if (Validate_checkAnn(NULL, ann, &d_blockHashes[index * 32], d_vctx)) {
            return Validate_checkBlock_ANN_INVALID(index);
        }
    }
    ...
    if (effectiveAnnTarget > d_coinbaseCommitment->annLeastWorkTarget) {
        return Validate_checkBlock_ANN_INSUF_POW(index);
    }
    Hash_COMPRESS32_OBJ(&d_annHashes[index], ann);  
}
                       ...
                       uint8_t workHashOut[static 32])
{
    ...
    CryptoCycle_State_t* d_pcState;
    uint64_t* d_indexesOut;
    PacketCrypt_HeaderAndProof_t* d_hap;
    cudaMalloc((void **)&d_pcState, sizeof(CryptoCycle_State_t));
    cudaMalloc((void **)&d_indexesOut, PacketCrypt_NUM_ANNS*sizeof(uint64_t));
    cudaMalloc((void **)&d_hap, sizeof(PacketCrypt_HeaderAndProof_t));

    cudaMemcpy(d_pcState, pcState, sizeof(CryptoCycle_State_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_indexesOut, indexesOut, PacketCrypt_NUM_ANNS*sizeof(uint64_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_hap, hap, sizeof(PacketCrypt_HeaderAndProof_t), cudaMemcpyHostToDevice);
    checkPcHashKernal<<<1, 4>>>(d_pcState, d_indexesOut, d_hap);
    cudaMemcpy(pcState, d_pcState, sizeof(CryptoCycle_State_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(indexesOut, d_indexesOut, PacketCrypt_NUM_ANNS*sizeof(uint64_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(hap, d_hap, sizeof(PacketCrypt_HeaderAndProof_t), cudaMemcpyDeviceToHost);
    cudaFree(d_pcState);
    cudaFree(d_indexesOut);
    cudaFree(d_hap);
                        ...
                        PacketCrypt_ValidateCtx_t* vctx)
{
    ...
    PacketCrypt_HeaderAndProof_t* d_hap;
    uint8_t* d_blockHashes;
    Buf32_t* d_annHashes;  
    PacketCrypt_ValidateCtx_t* d_vctx;
    PacketCrypt_Coinbase_t* d_coinbaseCommitment;

    cudaMalloc((void **)&d_hap, sizeof(PacketCrypt_HeaderAndProof_t));
    cudaMalloc((void **)&d_blockHashes, PacketCrypt_NUM_ANNS * 32*sizeof(uint8_t));
    cudaMalloc((void **)&d_annHashes, PacketCrypt_NUM_ANNS*sizeof(Buf32_t));
    cudaMalloc((void **)&d_vctx, sizeof(PacketCrypt_ValidateCtx_t));
    cudaMalloc((void **)&d_coinbaseCommitment, sizeof(PacketCrypt_Coinbase_t));

    cudaMemcpy(d_hap, hap, sizeof(PacketCrypt_HeaderAndProof_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_blockHashes, blockHashes, PacketCrypt_NUM_ANNS * 32*sizeof(uint8_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_annHashes, annHashes, PacketCrypt_NUM_ANNS*sizeof(Buf32_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_vctx, vctx, sizeof(PacketCrypt_ValidateCtx_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_coinbaseCommitment, coinbaseCommitment, sizeof(PacketCrypt_Coinbase_t), cudaMemcpyHostToDevice);

    Validate_checkBlockKernal<<<1, PacketCrypt_NUM_ANNS>>>(d_hap, d_blockHashes, d_annHashes, d_vctx, d_coinbaseCommitment, blockHeight);        
    cudaMemcpy(hap, d_hap, sizeof(PacketCrypt_HeaderAndProof_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(blockHashes, d_blockHashes, PacketCrypt_NUM_ANNS * 32*sizeof(uint8_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(annHashes, d_annHashes, PacketCrypt_NUM_ANNS*sizeof(Buf32_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(vctx, d_vctx,  sizeof(PacketCrypt_ValidateCtx_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(coinbaseCommitment, d_coinbaseCommitment, sizeof(PacketCrypt_Coinbase_t), cudaMemcpyDeviceToHost);    
    ...
    cudaFree(d_hap);
    cudaFree(d_blockHashes);
    cudaFree(d_annHashes);
    cudaFree(d_vctx);
    cudaFree(d_coinbaseCommitment);
