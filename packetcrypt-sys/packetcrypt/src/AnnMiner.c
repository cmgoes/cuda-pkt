...
__global__ void AnnMiner_freeKernal(AnnMiner_t* a) {
    int index = threadIdx.x + blockIdx.x * blockDim.x;
    setRequestedState(a, &a->workers[index], ThreadState_SHUTDOWN);    
    pthread_cond_broadcast(&a->cond);
    while (!threadsStopped(a)) { Time_nsleep(100000); }    
    assert(!pthread_join(a->workers[index].thread, NULL));    
}
...
{
    AnnMiner_t* d_ctx;
    cudaMalloc((void **)&d_ctx, sizeof(AnnMiner_t));
    cudaMemcpy(d_ctx, ctx, sizeof(AnnMiner_t), cudaMemcpyHostToDevice);
    AnnMiner_freeKernal<<<1, ctx->numWorkers>>>(d_ctx);
    cudaMemcpy(ctx, d_ctx, sizeof(AnnMiner_t), cudaMemcpyDeviceToHost);
    cudaFree(d_ctx);
    freeCtx(ctx);
}