...
__global__ void BlockMine_fakeMineKernal (BlockMine_pvt_t* d_ctx,  CryptoCycle_state_t* d_pcState, PacketCrypt_BlockHeader_t* d_hdr, 
Buf32_t* d_hdrHash, BlockMine_Rest_t* d_res, uint32_t* d_annIndexes, uint32_t lowNonce, uint32_t annCount) {
    while (1) {
        CryptoCycle_init(&d_pcState, &d_hdrHash, ++lowNonce);
        int index = threadIdx.x + blockIdx.x * blockDim.x;
        uint64_t itnum = d_res->ann_llocs[index] = CryptoCycle_getItemNo(&d_pcState) % annCount;
        assert(itnum < annCount);
        uint64_t x = d_res->ann_mlocs[index] = d_annIndexes[itnum];
        CryptoCycle_Item_t* it = (CryptoCycle_Item_t*) &d_ctx->g.anns[x];
        assert(CryptoCycle_update(&d_pcState, it));
        
        CryptoCycle_smul(&d_pcState);
        CryptoCycle_final(&d_pcState);
        if (!Work_check(d_pcState.bytes, 0x207fffff)) { continue; }
        break;
    }
}
    ...
    const uint32_t* annIndexes)
{
    ...
    BlockMine_pvt_t* d_ctx;
    CryptoCycle_state_t* d_pcState;
    PacketCrypt_BlockHeader_t* d_hdr;
    Buf32_t* d_hdrHash;
    BlockMine_Rest_t* d_res;
    const uint32_t* d_annIndexes;
    cudaMalloc((void **)&d_ctx, sizeof(BlockMine_pvt_t));
    cudaMalloc((void **)&d_pcState, sizeof(CryptoCycle_state_t));
    cudaMalloc((void **)&d_hdr, sizeof(PacketCrypt_BlockHeader_t));
    cudaMalloc((void **)&d_hdrHash, sizeof(BlockMine_pvt_t));
    cudaMalloc((void **)&d_res, sizeof(BlockMine_Res_t));
    cudaMalloc((void **)&d_annIndexes, sizeof(uint32_t));
    cudaMemcpy(d_ctx, ctx, sizeof(BlockMine_pvt_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_pcState, pcState, sizeof(CryptoCycle_state_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_hdr, hdr, sizeof(PacketCrypt_BlockHeader_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_hdrHash), hdrHash, sizeof(BlockMine_pvt_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_res), res, sizeof(BlockMine_Res_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_annIndexes), annIndexes, sizeof(uint32_t), cudaMemcpyHostToDevice);
    BlockMine_fakeMineKernal<<<1, 4>>>(d_ctx, d_pcState, d_hdr, d_hdrHash, d_res, d_annIndexes, lowNonce, annCount);
    cudaMemcpy(ctx, d_ctx, sizeof(BlockMine_pvt_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(pcState, d_pcState, sizeof(CryptoCycle_state_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(hdr, d_hdr, sizeof(PacketCrypt_BlockHeader_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(hdrHash, d_hdrHash, sizeof(BlockMine_pvt_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(res, d_res, sizeof(BlockMine_Res_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(annIndexes, d_annIndexes, sizeof(uint32_t), cudaMemcpyDeviceToHost);
    cudaFree(d_ctx);
    cudaFree(d_pcstate);
    cudaFree(d_hdr);
    cudaFree(d_hdrHash);
    cudaFree(d_res);
    cudaFree(d_annIndexes);
    res->high_nonce = hdr.nonce;
    res->low_nonce = lowNonce;
}